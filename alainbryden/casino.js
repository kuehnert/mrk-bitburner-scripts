...
const ran_flag = 'alainbryden/Temp/ran-casino.txt';
...
export async function main(ns) {
  ...
  if (ns.getPlayer().city != 'Aevum') {
    try {
      if (
        ...
        !(await getNsDataThroughFile(ns, 'ns.travelToCity("Aevum")', '/Temp/travel-to-city.txt'))
      )
  ...
  const checkForFocusScreen = () =>
    find("//button[contains(text(), 'Stop playing')]")
  ...
  try {
    ...
    try {
      await getNsDataThroughFile(ns, 'ns.goToLocation("Iker Molina Casino")', '/Temp/go-to-location.txt');
    } catch {
      return ns.tprint('ERROR: Failed to travel to the casino both using UI navigation and using SF4 as a fall-back.');
    }
  }
...
async function killAllOtherScripts(ns, removeRemoteFiles) {
  ...
  const otherPids = ns
    .ps()
    .filter(p => p.filename != thisScript)
    .map(p => p.pid);
  let pid = await runCommand(ns, 'ns.args.forEach(pid => ns.kill(pid))', '/Temp/kill-scripts-by-id.js', otherPids);
