...
export async function main(_ns) {
  ...
  var weakenThreads = maxThreads - ns.getServerMinSecurityLevel(server) / 0.05;
  var maxGrowThreads = maxRam / growscriptRam - weakenscriptRam * maxThreads;
  ...
  if (ma < mm) {
    ns.exec('/newserver/weaken.js', server2, maxThreads, server, 0);
  ...
  while (true) {
    ...
    if (totalRamForRun < maxRam - UsedRam) {
      ...
      ns.printf('totalRamForRun: %s', totalRamForRun);
      ns.printf('available: %s', maxRam - UsedRam);
